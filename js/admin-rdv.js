/* ============================================================
   ...
   ============================================================ */
    ...
    `;
              ...
              return `
                <div style="display:flex;align-items:center;justify-content:space-between;padding:.75rem 1rem;background:var(--bg-card, #fff);border-radius:10px;margin-bottom:.5rem;border-left:4px solid ${statutBadge.color};flex-wrap:wrap;gap:.5rem">
                  ...
                  <div style="display:flex;gap:.4rem;flex-shrink:0">
                    ...
                    ${phone ? `<a href="tel:${phone}" class="btn btn-sm btn-outline" style="padding:4px 8px;font-size:.75rem" title="Appeler"><i class="fas fa-phone"></i></a>` : ''}
        ...
        g.innerHTML = `<div style="color:var(--danger);padding:1rem">Erreur : ${e.message}</div>`;
      }
    }
    ...
    async function loadRdv(statut) {
      ...
      try {
        ...
        for (const r of allConfirmes) {
          ...
          const rdvDate = new Date(r.date_rdv + 'T23:59:59');
          if (rdvDate < today) {
      ...
      <div class="table-wrap">
        <table class="data-table">
          ...
          const dateStr = r.date_rdv ? new Date(r.date_rdv + 'T12:00:00').toLocaleDateString('fr-FR', { day: '2-digit', month: 'short' }) : '—';
          return `
                  ...
                  <td colspan="5">
                    <div style="display:flex;gap:2rem;flex-wrap:wrap">
                      <div style="flex:1;min-width:300px">
                        ...
                        <p><strong>Téléphone :</strong> <a href="tel:${r.tel}">${r.tel}</a></p>
                        ${r.email ? `<p><strong>Email :</strong> <a href="mailto:${r.email}">${r.email}</a></p>` : ''}
                        <p><strong>Date :</strong> ${r.date_rdv} à ${r.heure || 'non précisée'}</p>
                      ...
                      ${r.photo_url ? `
                        <div style="flex:0 0 200px">
                          ...
                          <a href="${r.photo_url}" target="_blank">
                            <img src="${r.photo_url}" class="photo-preview-admin" alt="Photo client">
                          </a>
                        </div>
                      ` : ''}
                    </div>
        ...
        el.innerHTML = `<div style="color:var(--danger);padding:1rem">Erreur : ${e.message}</div>`;
      }
    }
    ...
    function isToRelance(r) {
      ...
      const rdvDate = new Date(r.date_rdv + 'T12:00:00');
    ...
    window.openModal = (name) => {
      if (name === 'blog') openModal_Blog_new();
      else document.getElementById(`modal-${name}`).classList.add('open');
    };
