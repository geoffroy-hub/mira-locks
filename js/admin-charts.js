...
function renderTodayAgenda(rdvs) {
    ...
    let itemsHtml = todayRdvs.map(r => {
        ...
        <div style="display:flex; align-items:center; gap:1rem; padding:1rem 0; border-bottom:1px solid #f0f0f0; opacity:${isPast ? '0.6' : '1'}">
          <div style="font-family:'Playfair Display'; font-weight:700; color:var(--or); font-size:1.1rem; min-width:60px">${r.heure || '—'}</div>
          <div style="flex:1">
            <div style="font-weight:600; color:var(--vert)">${r.nom}</div>
            <div style="font-size:.75rem; color:#777">${r.service || 'Prestation'}</div>
          </div>
          <div style="display:flex; gap:.5rem">
            ${tel ? `<a href="tel:${tel}" class="btn btn-sm btn-outline" style="padding:.25rem .5rem"><i class="fas fa-phone"></i></a>` : ''}
            ${tel ? `<a href="https://wa.me/${tel}?text=${waMsg}" target="_blank" class="btn btn-sm btn-outline" style="border-color:#25D366; color:#25D366; padding:.25rem .5rem"><i class="fab fa-whatsapp"></i></a>` : ''}
          </div>
        </div>`;
    }).join('');
